...
WFD.name <- file.path(WFDPath,"Daily_Climate/WFD_PE_daily_2d_Europe_wide.nc")
WFDEI.PET.name <- file.path(WFDEIPath,"Daily_Climate/New/WFDEI_PET_PM_norad_2d.nc")
...
###########################################################################
# Read in WFD
WFD_PET.nc <- nc_open(WFD.name)
WFD_PET.nc
WFD_PET.mat <- ncvar_get(WFD_PET.nc, "PET")
WFD.landid <- ncvar_get(WFD_PET.nc, "land")
nc_close(WFD_PET.nc)
rm(WFD_PET.nc)

### Extract WFD to Wide size
WFD_PET.mat.Europe <- matrix(WFD_PET.mat[WFD.landid %in% LandInEurope],length(LandInEurope),dim(WFD_PET.mat)[2])
#### Extract the bad parts
WFD_PET.mat.Edit <- WFD_PET.mat.Europe[LandEdit,]

rm(WFD_PET.mat)
rm(WFD_PET.mat.Europe)

# Read in WFDEI PET
WFDEI_PET.nc <- nc_open(WFDEI.PET.name)
WFDEI_PET.nc
WFDEI_PET.mat <- ncvar_get(WFDEI_PET.nc, "PET")
WFDEI.landid <- ncvar_get(WFDEI_PET.nc, "landid")
nc_close(WFDEI_PET.nc)
rm(WFDEI_PET.nc)
 
 ### Extract WFDEIEI to Wide size
WFDEI_PET.mat.Europe <- matrix(WFDEI_PET.mat[WFDEI.landid %in% LandInEurope],length(LandInEurope),dim(WFDEI_PET.mat)[2])
#### Extract the bad parts
WFDEI_PET.mat.Edit <- WFDEI_PET.mat.Europe[LandEdit,]
rm(WFDEI_PET.mat)
rm(WFDEI_PET.mat.Europe)


## Test plot
plot(WFD.noleap,WFD_PET.mat.Edit[3000,], type="l", xlim=c(as.Date("1955-01-01"), as.Date("2015-01-01")))
lines(WFDEI.noleap,WFDEI_PET.mat.Edit[3000,], col="red")


###########################################################################
## Run everything for PET
###########################################################################
...
WFD.accum <- t(apply(WFD_PET.mat.Edit,1,roll_mean, n=183, fill=NA, align="right"))
WFDEI.accum <- t(apply(WFDEI_PET.mat.Edit,1,roll_mean, n=183, fill=NA, align="right"))

#WFD.accum <- t(parApply(cluster, WFD_PET.mat.Edit,1,roll_mean, n=183, fill=NA, align="right"))
#WFDEI.accum <- t(parApply(cluster, WFDEI_PET.mat.Edit,1,roll_mean, n=183, fill=NA, align="right"))
Sys.time()
...
#write.csv(Plot.df, "PET_TimeSeriesPlot_Europe.csv",row.names=FALSE)
...
write.csv(Plot.df, "PET_Option1_Europe.csv",row.names=FALSE)
...
write.csv(Plot.df, "PET_Option2_Europe.csv",row.names=FALSE)
...
write.csv(Quant.df, "PET_Option3_Europe.csv",row.names=FALSE)
...
write.csv(Plot.df, "PET_Option4_Europe.csv",row.names=FALSE)
