...
#################################################
#### Read in SPI
Tdiff.df <- read.csv("Tdiff_Option2_Europe.csv")
Tdiff.df$Date <- as.Date(Tdiff.df$Date)

Tdiff.df$Data <- factor(Tdiff.df$Data)
Tdiff.df$YearFrac <- as.numeric(Tdiff.df$Date)
Tdiff.df$Year <- as.numeric(substr(Tdiff.df$Date,1,4))
Tdiff.df$Month <- as.numeric(substring(Tdiff.df$Date,6,7))

monthly.test <- Tdiff.df$Date %in% monthly.dates
Tdiff.monthly.df <- Tdiff.df[monthly.test,]
...
### Fit plain model
m1 <- gamm(Value ~ s(YearFrac, k = 10) + Data, data = Tdiff.monthly.df,   control = ctrl, method="REML")
summary(m1$gam)
plot(m1$gam, residuals=TRUE)

## Account for potential annual cycle
m2 <- gamm(Value ~ s(YearFrac, k = 10) + s(Month, bs = "cc", k = 12) + Data, data = Tdiff.monthly.df,   control = ctrl, method="REML")
summary(m2$gam)
...
## Fit the AR1
m3.monthly <- try(gamm(Value ~ s(YearFrac, k = 10) + s(Month, bs = "cc", k = 12) + Data, data = Tdiff.monthly.df, correlation = corARMA(form = ~ 1|Year, p = 1),   control = ctrl, method="REML"))
## ...and fit the AR2
m4.monthly <- try(gamm(Value ~ s(YearFrac, k = 10) + s(Month, bs = "cc", k = 12) + Data, data = Tdiff.monthly.df, correlation = corARMA(form = ~ 1|Year, p = 2),   control = ctrl, method="REML"))
## ...and fit the AR3
m5.monthly <- try(gamm(Value ~ s(YearFrac, k = 10) + s(Month, bs = "cc", k = 12) + Data, data = Tdiff.monthly.df, correlation = corARMA(form = ~ 1|Year, p = 3),   control = ctrl, method="REML"))
...
## Fit without data
m3.nodata <- try(gamm(Value ~ s(YearFrac, k = 10) + s(Month, bs = "cc", k = 12) , data = Tdiff.monthly.df, correlation = corARMA(form = ~ 1|Year, p = 1),   control = ctrl, method="REML"))
## ...and fit the AR2
m4.nodata <- try(gamm(Value ~ s(YearFrac, k = 10) + s(Month, bs = "cc", k = 12) , data = Tdiff.monthly.df, correlation = corARMA(form = ~ 1|Year, p = 2),   control = ctrl, method="REML"))
## ...and fit the AR3
m5.nodata <- try(gamm(Value ~ s(YearFrac, k = 10) + s(Month, bs = "cc", k = 12), data = Tdiff.monthly.df, correlation = corARMA(form = ~ 1|Year, p = 3),   control = ctrl, method="REML"))
...
just.WFD <- subset(Tdiff.df, Data=="WFD")
...
just.WFDEI <- subset(Tdiff.df, Data=="WFDEI")
...
Pred.comb <- Tdiff.monthly.df[!duplicated(Tdiff.monthly.df$Date),]
Pred.comb$Data <- "WFD"
...
p <- ggplot(Tdiff.df, aes(x=Date, group=Data))
...
p <- p + geom_segment(aes(x = as.Date("1971-01-01"), y = -0.69, xend = as.Date("2000-12-31"), yend = -0.69), size=2, alpha=0.8, colour="grey80")
p <- p + scale_y_continuous(breaks=seq(-1,1,0.2), name="European 6 month Tmax - Tmin anomaly (C)")
p <- p + theme_classic(9)
p <- p + theme(legend.position=c(0.25,0.85), legend.direction="horizontal")
p <- p + scale_colour_brewer(type="qual", palette="Set1")
p <- p + coord_cartesian(ylim=c(-0.72, 0.73))
p

ggsave("Tdiff6_Version1.png", p, width=7, height=4, dpi=400)
ggsave("Tdiff6_Version1.pdf", p, width=7, height=4)

p <- ggplot(Tdiff.df, aes(x=Date, group=Data))
p <- p + geom_hline(yintercept=0, linetype="longdash", colour="grey30")
#p <- p + geom_line(aes(y=Europe), colour="grey70", size=0.3)
p <- p + geom_line(aes(y=Value, colour=Data), size=0.3,alpha=0.7)
p <- p + geom_line(data=Pred.comb, aes(y=Pred), size = 0.75, colour="grey20", linetype="dashed")
p <- p + geom_line(data=Pred.comb, aes(y=Inc), colour="black", size=1)
p <- p + geom_line(data=Pred.comb, aes(y=Dec), colour="black", size=1)
p <- p + geom_segment(aes(x = as.Date("1971-01-01"), y = -0.69, xend = as.Date("2000-12-31"), yend = -0.69), size=2, alpha=0.8, colour="grey80")
p <- p + scale_y_continuous(breaks=seq(-1,1,0.2), name="European 6 month Tmax - Tmin anomaly (C)")
p <- p + theme_classic(9)
p <- p + theme(legend.position=c(0.25,0.85), legend.direction="horizontal")
...
ggsave("Tdiff6_Version2.png", p, width=7, height=4, dpi=400)
ggsave("Tdiff6_Version2.pdf", p, width=7, height=4)



p <- ggplot(Tdiff.monthly.df, aes(x=Date, group=Data))
p <- p + geom_hline(yintercept=0, linetype="longdash", colour="grey30")
#p <- p + geom_line(aes(y=Europe), colour="grey70", size=0.3)
...
p <- p + geom_segment(aes(x = as.Date("1971-01-01"), y = -0.69, xend = as.Date("2000-12-31"), yend = -0.69), size=2, alpha=0.8, colour="grey80")
p <- p + scale_y_continuous(name=expression(paste("6 month ",T[max], " - ", T[min],"  anomaly (C)", sep="")))
...
ggsave("Tdiff6_Version2_poster2.png", p, width=7, height=4, dpi=400)
ggsave("Tdiff6_Version2_poster.pdf", p, width=7, height=4)
...
ggsave("Tdiff6_Version3.png", p, width=7, height=4, dpi=400)
ggsave("Tdiff6_Version3.pdf", p, width=7, height=4)
