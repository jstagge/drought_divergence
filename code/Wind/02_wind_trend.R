...
#################################################
#### Read in SPI
Wind.df <- read.csv("Wind_Option2_Europe.csv")
Wind.df$Date <- as.Date(Wind.df$Date)

Wind.df$Data <- factor(Wind.df$Data)
Wind.df$YearFrac <- as.numeric(Wind.df$Date)
Wind.df$Year <- as.numeric(substr(Wind.df$Date,1,4))
Wind.df$Month <- as.numeric(substring(Wind.df$Date,6,7))

monthly.test <- Wind.df$Date %in% monthly.dates
Wind.monthly.df <- Wind.df[monthly.test,]
...
### Fit plain model
m1 <- gamm(Value ~ s(YearFrac, k = 10) + Data, data = Wind.monthly.df,   control = ctrl, method="REML")
summary(m1$gam)
plot(m1$gam, residuals=TRUE)

## Account for potential annual cycle
m2 <- gamm(Value ~ s(YearFrac, k = 10) + s(Month, bs = "cc", k = 12) + Data, data = Wind.monthly.df,   control = ctrl, method="REML")
summary(m2$gam)
...
## Fit the AR1
m3.monthly <- try(gamm(Value ~ s(YearFrac, k = 10) + s(Month, bs = "cc", k = 12) + Data, data = Wind.monthly.df, correlation = corARMA(form = ~ 1|Year, p = 1),   control = ctrl, method="REML"))
## ...and fit the AR2
m4.monthly <- try(gamm(Value ~ s(YearFrac, k = 10) + s(Month, bs = "cc", k = 12) + Data, data = Wind.monthly.df, correlation = corARMA(form = ~ 1|Year, p = 2),   control = ctrl, method="REML"))
## ...and fit the AR3
m5.monthly <- try(gamm(Value ~ s(YearFrac, k = 10) + s(Month, bs = "cc", k = 12) + Data, data = Wind.monthly.df, correlation = corARMA(form = ~ 1|Year, p = 3),   control = ctrl, method="REML"))
...
just.WFD <- subset(Wind.df, Data=="WFD")
...
just.WFDEI <- subset(Wind.df, Data=="WFDEI")
...
p <- ggplot(Wind.monthly.df, aes(x=Date, group=Data))
...
p <- p + geom_segment(aes(x = as.Date("1971-01-01"), y = -0.33, xend = as.Date("2000-12-31"), yend = -0.33), size=2, alpha=0.8, colour="grey80")
p <- p + scale_y_continuous(name="European 6 month Wind anomaly (mm/s at 2m)")
p <- p + theme_classic(9)
p <- p + theme(legend.position=c(0.25,0.85), legend.direction="horizontal")
p <- p + scale_colour_brewer(type="qual", palette="Set1")
p <- p + coord_cartesian(ylim=c(-0.35, 0.63))
p

ggsave("Wind6_Version1.png", p, width=7, height=4, dpi=400)
ggsave("Wind6_Version1.pdf", p, width=7, height=4)
...
ggsave("Wind6_Version2.png", p, width=7, height=4, dpi=400)
ggsave("Wind6_Version2.pdf", p, width=7, height=4)
...
p <- p + scale_y_continuous(name="6 month Wind anomaly (m/s at 2m)")
...
ggsave("Wind6_Version2_poster.png", p, width=7, height=4, dpi=400)
ggsave("Wind6_Version2_poster.pdf", p, width=7, height=4)
...
ggsave("Wind6_Version3.png", p, width=7, height=4, dpi=400)
ggsave("Wind6_Version3.pdf", p, width=7, height=4)
...
#################################
just.WFDEI <- subset(Wind.monthly.df, Data=="WFDEI")
just.WFDEI$Data <- "WFD"
just.WFDEI$Month <- zero.point
...
Wind.df.adj <- rbind(just.WFD, just.WFDEI)

Pred.WFDEI <- data.frame(Date=just.WFDEI$Date, Pred=p1, Inc=S$incr, Dec=S$decr, Data="WFDEI")
Pred.df <- rbind(Pred.WFD, Pred.WFDEI)


which.max(Wind.df.adj$Value)
which.min(Wind.df.adj$Value[1:8000])
Wind.df.adj[16257,]
which.max(Pred.df$Pred)
Pred.df[16257,]
 
p <- ggplot(Wind.df.adj, aes(x=Date, group=Data))
p <- p + geom_hline(yintercept=0, linetype="longdash", colour="grey30")
p <- p + geom_line(aes(y=Value), colour="grey70", size=0.3)
#p <- p + geom_line(aes(y=Europe, colour=Data), size=0.3,alpha=0.7)
p <- p + geom_line(data=Pred.df, aes(y=Pred), colour="black", size = 0.75, colour="grey20")
...
p <- p + theme(legend.position=c(0.25,0.85), legend.direction="horizontal")
p <- p + scale_colour_brewer(type="qual", palette="Set1")
p <- p + coord_cartesian(ylim=c(-0.35, 0.27))
p

ggsave("Wind6_Version1_adj.png", p, width=7, height=4, dpi=400)
ggsave("Wind6_Version1_adj.pdf", p, width=7, height=4)
...
ggsave("Wind6_Version2_adj.png", p, width=7, height=4, dpi=400)
ggsave("Wind6_Version2_adj.pdf", p, width=7, height=4)
...
ggsave("Wind6_Version3_adj.png", p, width=7, height=4, dpi=400)
ggsave("Wind6_Version3_adj.pdf", p, width=7, height=4)
