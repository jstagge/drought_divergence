...
Deriv <- function(mod, n = 200, eps = 1e-7, newdata) {
	...
	## I added this to remove the factor "data"
	m.terms <-  m.terms[m.terms != "Data"]
	...
	col.index <- colnames(newD) %in% m.terms
    newD[,col.index] <- newD[,col.index] + eps
    ...
    lD$eps <- eps
	evalD <- newD
	evalD[,col.index] <- evalD[,col.index] - eps
    lD$eval <- evalD
    return(lD)
}
